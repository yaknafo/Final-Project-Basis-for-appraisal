...
namespace BasisForAppraisal_finalProject.BL
{
    public class ReportBL
    {
        ...
        public void CalculateReportForUnit(int companyId, int formId, string unit, List<string> empIds)
        {
            DataManager DMO = new DataManager();
            ...
            var report = new ReportForUnit{ companyId = companyId, FormId = formId, unitName = unit };
            DMO.SaveReportForUnit(report);
            ...
            var listOfReportLInes = new List<ReportForUnitLine>();
            var listOfReportLInesAnswers = new List<ReportForUnitMultipleChoiceListAnswer>();
            var allQuestions = form.Sections.First().Questions;

            foreach (tbl_IntentionalQuestion question in allQuestions)
            {
                var line = DMO.GetReportUnitLine(report.FormId, report.companyId, report.unitName, question.QuestionId);
                if (line == null)
                {
                    line = new ReportForUnitLine()
                    {
            ...
            // init All counters 
            foreach (ReportForUnitLine line in listOfReportLInes)
            {
                line.HighScore = 0;
                line.LowScore = 0;
                line.MidScore = 0;
            ...
            foreach (ReportForIndividualLine r in individualReportLines)
            {
                ...
                CountScoreForReportForUnitLines(r, line);
            ...
            foreach (tbl_IntentionalQuestion q in MulitiChoiceListQuestion)
            {
                var AnswerForQuestion = DMO.ConnectorAnswers.Where(x => x.QuestionId == q.QuestionId && x.companyId == companyId && x.tbl_ConnectorFormFill.tbl_Employee.unitName == unit).ToList();
                var groupByEmp = AnswerForQuestion.GroupBy(x => x.employeeOnId).ToList();
                var answerGroupOrg = AnswerForQuestion.GroupBy(x => x.AnswerId).ToList();
                var line = listOfReportLInes.FirstOrDefault(x => x.QuestionId == q.QuestionId);
                foreach (var i in groupByEmp)
                {
                    CountScoreForReportForUnitLinesMulitiChoice(i.Count(), line);
                }
                ...
                foreach (var i in q.tbl_IntentionalAnswers)
                {
                    var answerLine = DMO.GetSingleLineReportForUnitMultipleChoiceListAnswer(formId, companyId, unit, i.AnswerId);
                    if (answerLine == null)
                    {
                        answerLine = new ReportForUnitMultipleChoiceListAnswer() { AnswerId = i.AnswerId, companyId = companyId, unitName = unit, FormId = formId, QuestionId = q.QuestionId, SectionId = q.SectionId };
                    ...
                    DMO.SaveReportForUnitMultipleChoiceListAnswer(answerLine);
                }
            }
            ...
            // ------------ Save all lines  for report For Organiztion
            listOfReportLInes.ForEach(x => DMO.SaveReportForUnitLines(x));
            //   -------------End Report individual for Organiztion use--------------------------------------------
        }
        ...
        public void CountScoreForReportForUnitLines(ReportForIndividualLine individual, ReportForUnitLine organiztion)
        {
            if (individual.TotalAverage >= 2.5)
                organiztion.HighScore++;
            else if (individual.TotalAverage >= 1.5)
        ...
        public void CountScoreForReportForUnitLinesMulitiChoice(int NumberOfTime, ReportForUnitLine organiztion)
        {
            if (NumberOfTime >= 4)
                organiztion.HighScore++;
            else if (NumberOfTime >= 2)
        ...
        public void CountScoreForReportForOrganiztionLinesSpecialCatgory(int NumberOfTime, ReportForUnitLine organiztion)
        {
            var helper = new ConvertAnswerHelperScore();
            var ConvertScore = helper.ConvertAnswerHelperScoreForReport(NumberOfTime);
            if (ConvertScore >= 2.5)
